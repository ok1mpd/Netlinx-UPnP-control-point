...
integer HTTP_MAX_RESPONSE_LENGTH = 160000
...
define_function long http_post_SOAP_ACTION(char resource[], char body[],char Action[]) {
    stack_var http_url url
    stack_var http_request request
...
define_function char[HTTP_MAX_REQUEST_LENGTH] http_build_request(char host[],
    ...
    ret = "ret, 'Accept-Language: cs-CZ,cs;q=0.8', $0d, $0a"
    ...
    if (request.body != '') {
	    ...
	    if (find_string(request.body,'<s:Envelope',1)){
	 ret = "ret, 'CONTENT-TYPE: text/xml; charset="utf-8"', $0d, $0a"
	 if(request.SoapAction='Browse'){
		ret = "ret, 'SOAPACTION: "urn:schemas-upnp-org:service:ContentDirectory:1#',request.SoapAction,'"', $0d, $0a"
	    }
	    else if(request.SoapAction='SetVolume'){
		ret = "ret, 'SOAPACTION: "urn:schemas-upnp-org:service:RenderingControl:1#',request.SoapAction,'"', $0d, $0a"
		}
	    else
		{
		ret = "ret, 'SOAPACTION: "urn:schemas-upnp-org:service:AVTransport:1#',request.SoapAction,'"', $0d, $0a"
		}
	 }
    }
    ...
    offline: {
        ...
        if (http_resource_in_use(resource_id)) {
            ...
            if (valid_response) {
                ...
                http_response_received(req_obj.seq, req_obj.host, response)
